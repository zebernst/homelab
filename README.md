...
DNS for `jptr.zebernst.dev` is answered in-cluster by CoreDNS [k8s_gateway](https://github.com/k8s-gateway/k8s_gateway) (Service `dns-jptr`, Tailscale MagicDNS hostname `jupiter-dns`). Tailscale split DNS for that zone points at `jupiter-dns`; UniFi and Cloudflare ExternalDNS both exclude `jptr.zebernst.dev` so they never publish competing records.
...
| `jptr.zebernst.dev` | `jupiter-dns` (`dns-jptr` in-cluster) | Canonical private zone; answers with the Tailscale Gateway address |
